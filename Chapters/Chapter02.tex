...
In order to address pseudotime uncertainty in a formal and coherent framework, probabilistic approaches using Gaussian Process Latent Variable Models (GPLVM) have been used recently as non-parametric models of pseudotime trajectories \cite{reid2015pseudotime,campbell2015bayesian,macaulay2016single}. These provide an explicit model of pseudotimes as latent embedded one-dimensional variables. These models can be fitted within a Bayesian statistical framework using priors on the pseudotimes \cite{reid2015pseudotime}, deterministic optimisation methods for approximate inference \cite{macaulay2016single} or Markov Chain Monte Carlo (MCMC) simulations allowing full posterior uncertainty in the pseudotimes to be determined \cite{campbell2015bayesian}.
...
Single-cell RNA-seq data is also known to exhibit a large number of \emph{dropouts} which manifest as zero-inflated the data (see e.g. \cite{kharchenko2014bayesian}). This is due to a failure to reverse-transcript low abundance mRNAs, resulting in zero counts. To account for this, Trapnell introduces a \emph{Tobit} likelihood that models the observed expression $y$ in terms of the true expression $y^*$ and a limit of detection $\lambda$ % \graffito{Typically $\lambda = 0$}
as
...
We first provide a brief  of the Gaussian Process Latent Variable Model \cite{Titsias2010}. The GPLVM uses a Gaussian Process to define a stochastic mapping between a low-dimensional latent space to a (typically) higher dimensional observation space. A Gaussian Process is characterised by a mean function describing the expected mapping between the latent and observation spaces and a covariance function that describes the covariance between the mapping function evaluated at any two arbitrary latent positions. The covariance function therefore acts to control the second-order statistics of the Gaussian Process and suitable choices can be designed to encourage properties such as smoothness, periodicity or other second-order features.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{gfx/ch2/figure6.png}
...
\subsection{A sigmoidal model of switch-like behaviour across pseudotime}
